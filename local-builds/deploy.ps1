...
node (Join-Path $PROJECT_PATH "local-builds" "google-play-deploy.js") "$PACKAGE_NAME" "$SERVICE_ACCOUNT_KEY" "$latestAab" "$playTrack"

if ($LASTEXITCODE -ne 0) { exit 1 }

# Deploy bem-sucedido - libera incremento de versionCode no próximo build
'{"deployed":true}' | Set-Content $STATE_FILE -Encoding UTF8
